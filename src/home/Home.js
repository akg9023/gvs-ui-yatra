...
export default function Home(props) {
    const dbUserData = props.dbUserData
    ...
    const responseGoogle = async (response) => {
        ...
        const found = dbUserData.filter((one)=>email==one.email)
        if(found){
            sessionStorage.setItem("userId",found[0].id)
            navigate("/yatraMemReg",{state:{dbUserData:dbUserData}})
        }
        else{
            //not reg in db
            setErrMsg("Not Authorized. Please register yourself on http://db.yatra.iskconhaldia.in")
        }
    }
    return(
        <>
